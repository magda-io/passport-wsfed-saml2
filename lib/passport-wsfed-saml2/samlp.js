...
Samlp.prototype = {
  ...
  decodeResponse: function(req) {
    var decoded = zlib.inflateRawSync(new Buffer(req.body['SAMLResponse'], 'base64')).toString(this.default_encoding);
    ...
    if (encoding && encodingMappings[encoding] && encodingMappings[encoding] !== this.default_encoding){
      ...
      decoded = zlib.inflateRawSync(new Buffer(req.body['SAMLResponse'], 'base64')).toString(encodingMappings[encoding]);
    }
